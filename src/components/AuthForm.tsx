    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
      {mode === "signup" && (
        <div className="space-y-2">
          ...
          <Input id="name" placeholder="Enter your full name" value={name} onChange={(e) => setName(e.target.value)} required className="h-12" />
        </div>
      )}
      ...
      {mode === "signup" && role !== "admin" && (
        <RadioGroup value={selectedRole} onValueChange={(value) => setSelectedRole(value as "homeowner" | "provider")} className="flex space-x-4">
      ...
      <Button type="submit" className="w-full h-12 text-white font-medium transition-all duration-300 bg-gradient-to-r from-doit-400 to-orange-500 hover:from-doit-500 hover:to-orange-600" disabled={isSubmitting}>
        {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin" /> : mode === "login" ? "Login" : "Sign Up"}
      </Button>
    </form>
  );
};
